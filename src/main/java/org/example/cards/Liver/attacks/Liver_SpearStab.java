...
public class Liver_SpearStab extends CustomCard
{
    //卡牌ID，命名规则: mod名:卡牌名
    public static final String ID = "Liver:SpearStab";
    //卡牌插画路径
    public static final String IMG_PATH = "images/CharacterImg/Cards/Liver_SpearStab.png";
    //固定，不要修改
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new com.megacrit.cardcrawl.actions.utility.SFXAction("ATTACK_DAGGER_6"));
        ...
        for (AbstractMonster mo : AbstractDungeon.getMonsters().monsters) {
            if (!mo.isDeadOrEscaped()&& !(mo==m)) {
                if(damage>0){
                    AbstractDungeon.actionManager.addToBottom(new SpearDamageAction(mo, new DamageInfo(p, this.damage, this.damageTypeForTurn)));
                    damage-=2;
                }
            ...
            if (!mo.isDeadOrEscaped()) {
                AbstractDungeon.actionManager.addToBottom(
                        new com.megacrit.cardcrawl.actions.common.ApplyPowerAction(
                                mo, p, new ChuangBuff(mo, this.magicNumber), this.magicNumber
                        )
                );
            }
        }
