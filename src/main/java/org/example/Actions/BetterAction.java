...
public class BetterAction extends AbstractGameAction {
    private DamageInfo info;

    public BetterAction(AbstractCreature target, DamageInfo info) {
        this.info = info;
        ...
        if (this.shouldCancelAction()) {
            ...
            if (this.isDone) {
                ...
                if (this.target.lastDamageTaken > 0) {
                    addToTop(new ApplyPowerAction(this.source,this.source, new VigorPower(this.source, this.target.lastDamageTaken), this.target.lastDamageTaken));
