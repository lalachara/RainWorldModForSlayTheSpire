...
import org.example.relics.Liver_RelicFruit;
import org.example.tools.ModConfig;

import java.util.ArrayList;

import static org.example.Character.SlugCat.PlayerColorEnum.Liver_CLASS;
import static org.example.Character.SlugCat.PlayerColorEnum.Monk_CLASS;


// 继承CustomPlayer类
public class Monk extends SlugCat implements CustomSavable<Monk.ModSaveData> {
    // 火堆的人物立绘（行动前）
...
//    private static final float[] LAYER_SPEED ;
    ...
    private static final CharacterStrings characterStrings = CardCrawlGame.languagePack.getCharacterString("Rainworld:Monk");

    //自定义变量


    public Monk(String name) {
        super(name, Monk_CLASS,2,4,5,3);
        ...
        BaseMod.addSaveField("Monk:ModSaveData", this);
    }
    ...
    public void onLoad(ModSaveData modSaveData) {
        if(modSaveData == null) {
            // 如果没有保存数据，则使用默认值
            modSaveData = new ModSaveData(2, 4, 0, 5, 0,false,false,false,false);
        }
